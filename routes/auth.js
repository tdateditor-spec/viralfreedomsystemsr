...
router.post('/login', async (req, res) => {
  ...
  if (!student.paid || student.status === 'inactive')
    return res
      .status(403)
      .json({
        error: 'Tài khoản chưa được kích hoạt. Vui lòng hoàn tất thanh toán.',
      });

  if (!student.password)
    return res
      .status(403)
      .json({ error: 'Tài khoản chưa có mật khẩu. Vui lòng liên hệ hỗ trợ.' });
...
router.post('/reset-password', async (req, res) => {
  ...
  if (!student || !student.reset_token_expires) {
    return res
      .status(400)
      .json({ error: 'Link đặt lại mật khẩu không hợp lệ' });
  }

  if (new Date(student.reset_token_expires) < new Date()) {
    return res
      .status(400)
      .json({ error: 'Link đã hết hạn. Vui lòng yêu cầu lại.' });
  }
