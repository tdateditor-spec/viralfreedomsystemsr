  ...
  try {
    ...
    if (sigHeader) {
      ...
      if (secret) {
        ...
        const sigHex = sigHeader.startsWith('sha256=')
          ? sigHeader.slice(7)
          : sigHeader;
        if (sigHex.toLowerCase() !== expected.toLowerCase()) {
          console.warn(
            '⚠️  Webhook: invalid signature — processing anyway (verify secret matches SePay dashboard)'
          );
        }
      }
    }
    ...
    if (student) {
      ...
      console.log(
        `✅ Kích hoạt tài khoản: ${student.email} | Mật khẩu tạm: ${tempPassword}`
      );
      ...
      try {
        ...
        console.log(
          `📧 Đã gửi email tới: ${student.email} | Khoá: ${courses.join(', ')}`
        );
      } catch (emailErr) {
...
async function notifyTelegram(payload, student) {
  ...
  const email =
    student?.email || payload.customerEmail || payload.customer_email || '—';
