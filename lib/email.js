...
const DRIVE_LINKS = {
  ...
  music:  process.env.DRIVE_MUSIC  || 'https://drive.google.com/drive/folders/1gABuzQcWkAd_qE8kOOEmWeB3w7zzngcz?usp=sharing',
  plugin: process.env.DRIVE_PLUGIN || 'https://drive.google.com/drive/folders/1ilKTDtLaWhrMVI_1H_VG5haAL6kFynic?usp=sharing',
}

// Nút "bulletproof" — dùng <table>/<td> thay vì flex/gradient trên <a>,
// vì Gmail (đặc biệt app mobile) không hỗ trợ tốt display:flex và có thể
// khiến vùng bấm của nút bị lệch/không bấm được.
function button(url, label, { bg = '#2563eb', fontSize = '15px', padding = '14px 48px', block = false } = {}) {
  return `
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="${block ? 'width:100%;' : ''}margin:0;">
      <tr>
        <td align="center" bgcolor="${bg}" style="border-radius:10px;">
          <a href="${url}" target="_blank" rel="noopener"
            style="display:inline-block;${block ? 'width:100%;box-sizing:border-box;' : ''}padding:${padding};font-family:Inter,Arial,sans-serif;font-size:${fontSize};font-weight:700;color:#ffffff;text-decoration:none;border-radius:10px;">
            ${label}
          </a>
        </td>
      </tr>
...
async function sendWelcomeEmail({ name, email, tempPassword, courses = ['edit'] }) {
  ...
  // Build danh sách tài nguyên Drive — chỉ music và plugin (video editing truy cập qua login)
  const driveItems = [
  ...
  const driveSection = driveItems.map(c => `
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-bottom:1px solid rgba(255,255,255,0.06);">
      <tr>
        <td style="padding:12px 0;font-size:13px;color:#e2e8f0;">${c.icon} ${c.label}</td>
        <td align="right" style="padding:12px 0;width:1%;white-space:nowrap;">
          ${button(DRIVE_LINKS[c.key], 'Truy cập →', { bg: '#1e40af', fontSize: '12px', padding: '7px 16px' })}
        </td>
      </tr>
    </table>
  `).join('')
...
<body style="margin:0;padding:0;background:#0f1117;font-family:Inter,Arial,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:#141520;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
    ...
    <div style="padding:36px 40px;">
      ...
      <!-- Step 2: Tài nguyên Drive (chỉ hiện khi có music/plugin) -->
      ${driveSection ? `
      <div style="margin-bottom:20px;">
        <p style="margin:0 0 10px;font-size:12px;font-weight:700;color:#34d399;text-transform:uppercase;letter-spacing:1px;">② Tài nguyên bổ sung của bạn</p>
      ...
      <!-- Step 3 (hoặc 2 nếu không có drive): Discord -->
      <div style="margin-bottom:28px;">
        <p style="margin:0 0 10px;font-size:12px;font-weight:700;color:#a78bfa;text-transform:uppercase;letter-spacing:1px;">${driveSection ? '③' : '②'} Tham gia cộng đồng Discord</p>
        <div style="background:#0f1117;border:1px solid rgba(167,139,250,0.25);border-radius:14px;padding:20px;text-align:center;">
          ...
          ${button(DISCORD_URL, 'Vào nhóm Discord →', { bg: '#7c3aed', fontSize: '14px', padding: '12px 32px' })}
        </div>
      </div>

      <!-- CTA -->
      <div style="text-align:center;margin-bottom:8px;">
        ${button(loginUrl, 'Vào học ngay →', { bg: '#2563eb' })}
        <p style="margin:10px 0 0;font-size:12px;color:#475569;">Đăng nhập bằng email và mật khẩu tạm ở trên</p>
      </div>
    </div>
  ...
  await transporter.sendMail({
    ...
    to:      email,
    subject: `🎉 Thanh toán thành công — Tài khoản & nhóm Discord của bạn`,
    html,
  })
  ...
  <div style="max-width:560px;margin:40px auto;background:#141520;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
    ...
    <div style="padding:36px 40px;">
      ...
      <div style="text-align:center;margin:32px 0;">
        ${button(resetUrl, 'Đặt lại mật khẩu →', { bg: '#2563eb' })}
      </div>
